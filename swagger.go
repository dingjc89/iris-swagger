...
func newConfig(configFns ...OptionConfig) *Config {
	config := Config{
...
func IrisWrapHandler(handler *webdav.Handler, option ...OptionConfig) iris.Handler {
	...
	return func(ctx iris.Context) {
		if ctx.Request().Method != http.MethodGet {
			_ = ctx.StopWithJSON(http.StatusMethodNotAllowed, nil)
			return
		}
		...
		switch path {
		case "":
			ctx.Redirect(fmt.Sprintf("%s/%s", matches[1], "index.html"), http.StatusMovedPermanently)
		case "index.html":
			_ = index.Execute(ctx.ResponseWriter(), config)
		case "doc.json":
